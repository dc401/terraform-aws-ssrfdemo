...
do
    ...
    PAYLOAD=$(cat update_workspace_test_vars.json | jq --arg AWS_SECRET_ACCESS_KEY "$AWS_SECRET_ACCESS_KEY_ID" '. | select(.data.attributes.key == "AWS_SECRET_ACCESS_KEY") | .data.attributes.value=$AWS_SECRET_ACCESS_KEY')
    curl --header "Authorization: Bearer $TFCLOUD_API_TOKEN" \
    --header "Content-Type: application/vnd.api+json" \
    --request POST \
    --data $PAYLOAD \
